...
export const generateInvoiceNumber = async (shopId: string): Promise<string> => {
  try {
    ...
    // Format: INV-SHOP-YYMMDD-XXX where XXX is the sequential number for the day
    const sequentialNumber = ((count || 0) + 1).toString().padStart(4, '0');
    const shopPrefix = shopId.slice(0, 4).toLowerCase();
    
    const invoiceNumber = `INV-${shopPrefix}-${year}${month}${day}-${sequentialNumber}`;
