...
BEGIN
  ...
  -- If this is a return, update product stock and sale_items
  IF modification_type = 'return' AND returned_items IS NOT NULL AND jsonb_array_length(returned_items) > 0 THEN
    ...
    LOOP
      ...
      IF (item_record->>'selected')::boolean = true AND (item_record->>'quantity')::integer < (item_record->>'originalQuantity')::integer THEN
        ...
        -- Get product_id and current returned_quantity from sale_items
        SELECT 
          product_id,
        ...
        IF product_id IS NOT NULL THEN
          -- Calculate return quantity (difference between original and new quantity)
          return_quantity := (item_record->>'originalQuantity')::integer - (item_record->>'quantity')::integer;
          
          -- Update returned_quantity in sale_items
          UPDATE sale_items 
          SET 
            returned_quantity = current_returned + return_quantity,
            updated_at = NOW()
          WHERE id = sale_item_id;
          ...
          -- Log the updates
          RAISE NOTICE 'Product % stock updated to %, Sale item % returned quantity updated to %', 
            product_id, current_stock, sale_item_id, (current_returned + return_quantity);
        END IF;
      END IF;
    END LOOP;
