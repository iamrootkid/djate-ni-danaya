...
export const InvoiceModifyDialog = ({ open, onClose, invoice, onModified }: InvoiceModifyDialogProps) => {
  ...
  const fetchSaleItems = async (saleId: string) => {
    try {
      ...
      console.error("Error in fetchSaleItems:", error);
      toast({
        title: "Error",
  ...
  const handleSubmit = async (values: ModificationFormValues) => {
    ...
    try {
      ...
      const { data: modificationResult, error: modificationError } = await supabase.rpc(
        'create_invoice_modification' as keyof Database['public']['Functions'],
        modificationData
      );
  ...
  const handleItemSelection = (index: number, selected: boolean) => {
    ...
    if (currentItems[index]) {
      currentItems[index] = {
        ...currentItems[index],
        selected,
        // Reset quantity to original if deselected
        quantity: selected ? currentItems[index].quantity : currentItems[index].originalQuantity
      };
  ...
  const handleQuantityChange = (index: number, quantity: number) => {
    ...
    if (currentItems[index]) {
      ...
      const newQuantity = Math.min(
        Math.max(0, quantity), // Don't allow negative numbers
                  ...
                  <FormControl>
                    <RadioGroup
                      onValueChange={(value) => {
                        ...
                        // Reset form values when changing type
                        if (value === "price") {
                    ...
                    >
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <Label htmlFor="return">Return Items</Label>
                      </div>
                      <div className="flex items-center space-x-3">
                        <RadioGroupItem value="other" id="other" />
                        <Label htmlFor="other">Other Modification</Label>
                      </div>
                    </RadioGroup>
                  </FormControl>
                  <FormMessage />
                ...
                <div className="border rounded-md p-3 space-y-2 max-h-60 overflow-y-auto">
                  {originalItems.map((item, index) => (
                    <div key={item.id} className="flex items-center space-x-2 py-2 border-b last:border-0">
                      <input
                        type="checkbox"
                        checked={form.getValues("returnedItems")?.[index]?.selected || false}
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-sm text-muted-foreground">
                          Price: {item.price.toLocaleString()} F CFA
                        </div>
                      </div>
                      <div className="flex items-center space-x-2">
                        <Label htmlFor={`qty-${item.id}`}>Keep:</Label>
                        <Input
                          id={`qty-${item.id}`}
                          type="number"
                          disabled={!form.getValues("returnedItems")?.[index]?.selected}
                          min={0}
                          max={item.originalQuantity}
                          value={form.getValues("returnedItems")?.[index]?.quantity || 0}
