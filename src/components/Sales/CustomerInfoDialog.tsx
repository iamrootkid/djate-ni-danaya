...
interface CustomerInfoDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (customerName: string, customerPhone: string) => Promise<{ invoiceNumber: string }>;
  isPending: boolean;
  cart: CartItem[];
  cartTotal: number;
}
...
export const CustomerInfoDialog = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Ensure all required data is present before showing preview
      const invoiceData = {
      ...
      if (!invoiceData.items.length || invoiceData.total <= 0) {
