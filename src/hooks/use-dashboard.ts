...
export const useDashboard = () => {
  ...
  const { shopId, isLoading: isLoadingShop } = useShopId();
  ...
  const {
    ...
    queryFn: async () => {
      ...
      // Get stock summary
      const { data: stockSummary, error: stockError } = await supabase.rpc(
        "get_stock_summary",
        { shop_id: shopId }
      );
      ...
      const { data: bestSellers, error: bestSellersError } = await supabase.rpc(
        ...
        { shop_id: shopId, limit: 5 }
      );

      if (bestSellersError) {
        throw bestSellersError;
      }
  ...
  const isLoading = isLoadingShop || isLoadingData;
